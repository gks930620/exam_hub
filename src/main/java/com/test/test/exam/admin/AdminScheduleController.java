...
public class AdminScheduleController {
    ...
    /**
     * 특정 시험의 등록된 일정 — 입력 전에 이미 있는지 확인하는 용도.
     *
    ...
    public ResponseEntity<List<AdminDtos.ScheduleRow>> list(@RequestParam Long certificateId) {
        ...
        List<AdminDtos.ScheduleRow> rows = examScheduleRepository
                .findByCertificateIdAndStatusOrderByExamStartDateAsc(certificateId, ScheduleStatus.ACTIVE)
    ...
    public ResponseEntity<AdminDtos.ScheduleRow> upsert(@Valid @RequestBody AdminDtos.UpsertRequest req) {
        ...
        ExamSchedule schedule = examScheduleRepository
                ...
                .orElseGet(() -> ExamSchedule.builder()
                        .certificate(cert).year(req.year()).round(req.round()).examType(type)
                        .status(ScheduleStatus.ACTIVE)
                        .build());

        // 매니저가 공고를 보고 넣은 값이다 — 추정치 경고가 붙으면 안 된다
        schedule.changeProvenance(com.test.test.exam.domain.ScheduleProvenance.MANUAL);
        schedule.applyFrom(
        ...
        notificationScheduleService.recalc(saved, true);

        log.info("[Admin] 수기 일정 저장 cert={} {}년 {}회 {}",
                cert.getName(), req.year(), req.round(), type);
    ...
    private void validate(AdminDtos.UpsertRequest r) {
        if (r.regStartAt() != null && r.regEndAt() != null && r.regStartAt().isAfter(r.regEndAt())) {
            throw new BusinessRuleException("접수 시작이 마감보다 늦습니다.");
        }
        if (r.examStartDate() != null && r.regEndAt() != null
                && r.regEndAt().toLocalDate().isAfter(r.examStartDate())) {
            throw new BusinessRuleException("접수 마감이 시험일보다 늦습니다.");
        }
        if (r.examStartDate() != null && r.resultDate() != null
                && r.examStartDate().isAfter(r.resultDate())) {
            throw new BusinessRuleException("시험일이 발표일보다 늦습니다.");
        }
        if (r.examStartDate() != null && r.examEndDate() != null
                && r.examStartDate().isAfter(r.examEndDate())) {
            throw new BusinessRuleException("시험 시작일이 종료일보다 늦습니다.");
        }
    }
    ...
    public static final class AdminDtos {
        ...
        public record UpsertRequest(
                ...
                @NotNull(message = "연도를 입력하세요.") Integer year,
                @NotNull(message = "회차를 입력하세요.") Integer round,
