 ...
 * 시행처 확인 / 수집 점검. 행동이 없는 시험은 대기(자동으로 올 것)·정상(앞으로 일정 있음)·상시다.
 *
    ...
    <>
      ...
      {data && data.items.length === 0 && (
        <div className="k-empty state">
          <span className="big">{bucket === 'TODO' && !q.trim() ? '지금 할 일이 없습니다' : '해당하는 시험이 없습니다'}</span>
          {bucket === 'TODO' && !q.trim() ? '수기 대상 시험이 전부 앞으로의 일정을 갖고 있습니다.' : '검색어나 탭을 바꿔 보세요.'}
        </div>
      )}
...
function RowView({ row, onEdit }: { row: OverviewRow; onEdit: () => void }) {
  ...
  return (
    <tr>
      ...
      <td>
        {row.action && act ? (
          <span className={`k-badge ${act.tone}`}>{row.actionLabel}</span>
        ) : row.waitingLabel ? (
          <span className="k-dim">{row.waitingLabel}</span>
        ) : row.bucket === 'ROLLING' ? (
          <span className="k-dim">대상 아님</span>
        ) : (
          <span className="k-badge k-badge--ok">{row.nextBadge === 'REG_OPEN' ? '접수 중' : '예정'}</span>
        )}
      </td>
      ...
      <td>
        {row.bucket === 'ROLLING' ? null
          : row.source === 'MANUAL' || row.action === 'VERIFY'
