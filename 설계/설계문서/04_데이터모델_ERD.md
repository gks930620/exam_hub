...
> **⚠️ 본문의 `app_user`·`fcm_token`·`slug`(pSEO)는 폐기다** — 회원 테이블은 `member`(소셜 provider+providerId),
> FCM 컬럼은 없고(알림은 이메일), slug 는 pSEO 가 아니라 내부 식별용이다. 현행 스키마는
> `com.test.test.exam.domain` 엔티티와 `08_계정과_커뮤니티.md` §2 가 기준이다(2026-09-08).
>
